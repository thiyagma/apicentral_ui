...
_onSortByClick(e){
  ...
  bookMarkClick(currentValue,bool,e) {
 //   debugger;
    console.log("bookmark cliked");
    var bookmarkIndex = -1;
    for(let i=0; i < this.state.bookmarks.length; i++) {
      if(currentValue.api_id == this.state.bookmarks[i].api_id){
        bookmarkIndex = i;
        break;
      }
    }
    ...
    var lbookmarks = this.state.bookmarks;
    
    if(bookmarkIndex == -1) {
      lbookmarks.push({api_id:currentValue.api_id,api_name:currentValue.title});
      this.setState({bookmarks:lbookmarks});
    }
    else {
      lbookmarks.splice(bookmarkIndex,1);
      this.setState({bookmarks:lbookmarks});
    }

    const {session: {email}, session: {api_id}} = this.props;
    if (currentValue != undefined && email != undefined) {
      this.props.dispatch(addMyBookMarks(email, currentValue.api_id, currentValue.title, bool));
    }
    //e.stopPropgation();
    e.stopPropagation();
  }
  ...
  let isShow=item != undefined? true : false;
    ...
    {
      let _className = isShow? 'bookmark-icon-fill': 'bookmark-icon';
      console.log("ListView " + _className);
       viewResult = (
      <div className="result" key={index} onClick={this.selectResult.bind(this, currentValue)}>
        <div>{currentValue.eprId}</div>
        
        <div className="title">
          <Bookmark className={_className} onClick={this.bookMarkClick.bind(this,currentValue,isShow)}/> 
          <span className="tile-title-h">{currentValue.title}</span>
          <span className="tile-title-p"> By: {extractAuthorFromEmail(currentValue.create_by)}</span>
         </div>
